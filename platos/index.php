...
$query = "SELECT SQL_CALC_FOUND_ROWS p.*, c.nombre as categoria_nombre 
          FROM Platillos p
          LEFT JOIN CategoriasMenu c ON p.categoria_id = c.categoria_id";
...
$query .= " ORDER BY p.nombre ASC LIMIT :offset, :limit";
...
try {
    ...
    die("Error al obtener platillos: " . $e->getMessage());
}
?>

<div class="container mt-5 pt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-utensils me-2"></i>Gestión de Platillos</h2>
        <a href="<?= $base_url ?>registrarplatillo.php" class="btn btn-primary">
    ...
    <div class="card mb-4 shadow-sm">
        <div class="card-body">
            <form method="get" action="" class="row g-3">
                <div class="col-md-6">
                    <label for="busqueda" class="form-label">Buscar platillo:</label>
                    <div class="input-group">                      
                        ...
                        <select class="form-select" name="criterio" style="max-width: 150px;">
                            ...
                            <option value="categoria" <?= $criterio === 'categoria' ? 'selected' : '' ?>>Categoría</option>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
                ...
                <table class="table table-striped table-hover data-table">
                    ...
                    <tbody>
                        <?php if(empty($platillos)): ?>
                            <tr>
                                <td colspan="10" class="text-center">No se encontraron platillos</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach($platillos as $platillo): ?>
                            <tr>
                                <td><?= htmlspecialchars($platillo['platillo_id']) ?></td>
                                <td><?= htmlspecialchars($platillo['nombre']) ?></td>
                                <td><?= !empty($platillo['categoria_nombre']) ? htmlspecialchars($platillo['categoria_nombre']) : 'Sin categoría' ?></td>
                                <td><?= !empty($platillo['descripcion']) ? htmlspecialchars($platillo['descripcion']) : 'N/A' ?></td>
                                <td>S/ <?= number_format($platillo['precio'], 2) ?></td>
                                <td><?= !empty($platillo['costo']) ? 'S/ ' . number_format($platillo['costo'], 2) : 'N/A' ?></td>
                                <td><?= !empty($platillo['tiempo_preparacion']) ? htmlspecialchars($platillo['tiempo_preparacion']) . ' min' : 'N/A' ?></td>
                                <td>
                                    <?php if(!empty($platillo['imagen_url'])): ?>
                                        <img src="<?= htmlspecialchars($platillo['imagen_url']) ?>" alt="Imagen platillo" style="max-width: 50px; max-height: 50px;">
                                    <?php else: ?>
                                        N/A
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="editar.php?id=<?= $platillo['platillo_id'] ?>" class="btn btn-sm btn-warning" title="Editar">
            ...
            <div class="text-muted text-center">
                Mostrando <?= count($platillos) ?> de <?= $total_registros ?> registros
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    $('.btn-eliminar').click(function() {
        ...
        $('#confirmarEliminar').off('click').on('click', function() {
            window.location.href = 'eliminar.php?id=' + platilloId;
        });
    });
});
</script>